...
static void SD_ForceReset(void);
static void SD_Recover(BYTE lun, const char *op, DWORD sector, UINT count);
...
static void SD_ForceReset(void)
{
  __HAL_SD_DISABLE_IT(&hsd1, SDMMC_IT_DATAEND | SDMMC_IT_DCRCFAIL | SDMMC_IT_DTIMEOUT |
...
static void SD_Recover(BYTE lun, const char *op, DWORD sector, UINT count)
{
  uint32_t error_code = hsd1.ErrorCode;
  HAL_SD_StateTypeDef hal_state = HAL_SD_GetState(&hsd1);
  HAL_SD_CardStateTypeDef card_state = HAL_SD_GetCardState(&hsd1);

  printf("[SDIO] %s failed: sector=%lu count=%u hal_err=0x%08lX hal_state=%lu card_state=%lu\r\n",
         op,
  ...
  if (BSP_SD_Init() == MSD_OK)
  {
    ...
    printf("[SDIO] recover ok: stat=0x%02X\r\n", Stat);
  }
  else
  {
    Stat = STA_NOINIT;
    printf("[SDIO] recover failed\r\n");
  }
}
...
DRESULT SD_read(BYTE lun, BYTE *buff, DWORD sector, UINT count)
{
  ...
  for (attempt = 0; attempt <= SD_IO_RETRY_COUNT; attempt++)
  {
                         ...
                         count, SD_TIMEOUT) == MSD_OK)
    {
      ...
      printf("[SDIO] read ready timeout: sector=%lu count=%u\r\n",
             (unsigned long)sector,
             (unsigned int)count);
    }

    SD_Recover(lun, "read", sector, count);
  }
...
DRESULT SD_write(BYTE lun, const BYTE *buff, DWORD sector, UINT count)
{
  ...
  for (attempt = 0; attempt <= SD_IO_RETRY_COUNT; attempt++)
  {
                          ...
                          count, SD_TIMEOUT) == MSD_OK)
    {
      ...
      printf("[SDIO] write ready timeout: sector=%lu count=%u\r\n",
             (unsigned long)sector,
             (unsigned int)count);
    }

    SD_Recover(lun, "write", sector, count);
  }
